...
public static class ByteUtils
{
    public static ushort ReadUInt16(byte[] data, ref int offset)
    {
        if (offset + 2 > data.Length)
            throw new ArgumentOutOfRangeException(nameof(offset), "Not enough data to read UInt16");
    ...
    public static uint ReadUInt32(byte[] data, ref int offset)
    {
        if (offset + 4 > data.Length)
            throw new ArgumentOutOfRangeException(nameof(offset), "Not enough data to read UInt32");
    ...
    public static byte[] ReadBytes(byte[] data, ref int offset, int length)
    {
        if (offset + length > data.Length)
            throw new ArgumentOutOfRangeException(nameof(offset), "Not enough data to read requested bytes");
